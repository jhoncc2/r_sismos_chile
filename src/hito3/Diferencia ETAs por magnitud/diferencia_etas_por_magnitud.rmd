...
```{r}
boxplot(mag5_3d$Aumento, mag5_7d$Aumento, mag5_14d$Aumento, mag5_30d$Aumento)
lines(x=c(0, 5), y=rep(0, 2), col="red", lwd=2)
```

```{r}
boxplot(mag6_3d$Aumento, mag6_7d$Aumento, mag6_14d$Aumento, mag6_30d$Aumento)
lines(x=c(0, 5), y=rep(0, 2), col="red", lwd=2)
```

```{r}
boxplot(mag7_3d$Aumento, mag7_7d$Aumento, mag7_14d$Aumento, mag7_30d$Aumento)
lines(x=c(0, 5), y=rep(0, 2), col="red", lwd=2)
```
...
x1 <- generar_datos_cluster(mag6_7d)
x2 <- generar_datos_cluster(mag6_14d)
x3 <- generar_datos_cluster(mag7_7d)
x4 <- generar_datos_cluster(mag7_14d)
...
Se puede observar que en general para los sismos de magnitud 7, se genera un aumento para cualquier magnitud, produndidad o región.
...
```{r}
# Recordar que x1 es magnitud 6 a 7 días de intervalo
hc.complete <- hclust(dist(x1), method = "complete")
hc.single <- hclust(dist(x1), method = "single")
hc.average <- hclust(dist(x1), method = "average")
