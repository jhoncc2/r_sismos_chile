...
# Introducción
Los sismos son un fenómeno natural muy presente en nuestro país, producen sacudidas bruscas y pasajeras en la corteza terrestre debido a la liberación de energía acumulada en forma de ondas sismicas. Este fenómeno es aún impredecible y dadas las consecuencias que puede llegar a tener un sismo de gran magnitud es importante no sólo intentar predecir su ocurrencia sino también evaluar el efecto que podrían provocar el sismo en otros ámbitos.

Algunas correlaciones con otros fenómenos han sido estudiadas, entre ellas, se indagó principalmente en 2 estudios: el primero, sobre la relación de los sismos con las lluvias monzónicas en el Himalaya, y el segundo, sobre la influencia del sol en ellos. Los resultados del primer estudio mostraron que la frecuencia de sismos en el Himalaya se veía disminuida con los eventos monzónicos. Del segundo estudio, se extrajo que los datos sugerían que la mayor cantidad de sismos ocurrían en horarios diurnos.

Por otro lado, otro ámbito donde los sismos también podrían presentar consecuencias es en las enfermedades transmitidas por alimentos, por lo que se decide indagar en este tema. El  poder determinar si existe una relación entre estos eventos podría ayudar a desplegar infraestructura y recursos después de un desastre, además de ayudar en la generación de políticas públicas que ayuden a mitigar efectos secundarios provocados por sismos.

Sin embargo, posterior a la exploración de los datos realizado en el Hito 1, se decidió que la hipótesis a trabajar debía ser diferente, esto debido a que los datos obtenidos no sugerían relaciones claras entre los eventos estudiados como para generar un predictor. En este entendido, se añade al conjunto de _dataset_ una base de datos de egresos hospitalarios, con el fin de direccionar la experimentación a hacia el aumento o la disminución de estos, incluyendo de la misma forma las enfermedades transmitidas por alimentos.
...
En base a lo expuesto en la Sección \ref(hipotesis-hito-1), se planteó una nueva hipótesis con nuevas configuraciones:
...
Al tratarse de una extensión y mejora del hito 2, no se realiza cambios en las hipótesis ni configuraciones realizadas.
...
Se pretende realizar una unión entre los datasets de sismos con las precipitaciones fluviales, eventos de salud y enfermedades transmitidas por alimentos mediante la fecha y su posición relativa de los eventos tanto sismos como ingresos a hospitales o reporte de transmición de enfermedad. Las posiciones de los eventos serán enmarcados en las regiones a las que pertenezcan los atributos de latitud y longitud que se poseen en los datasets. De esta manera se realizarán varias pruebas para encontrar posibles correlaciones entre los eventos estudiados con el fin de aceptar o rechazar las hipótesis. 
...
Dentro de los datos que se utilizarán para el análisis están: longitud, latitud, magnitud, profundidad, y fecha.
La longitud y latitud son parte del sistema de coordenadas geográficas, el cual es un sistema que referencia cualquier punto de la superficie terrestre y se utiliza para ello dos coordenadas angulares, latitud (norte o sur) y longitud (este u oeste). La Magnitud es una medida que tiene relación con la cantidad de energía liberada en forma de ondas y la profundad define la distancia entre el epicentro de un terremoto con respecto al nivel del mar, finalmente la fecha es el registro del evento.
...
Se deben declarar como NA los datos con -9999 que representan datos faltantes, para que R los trabaje de buena manera. Luego se aplica el promedio de todos los años desde 1900 a 2018, evitando los NA.
...
En el siguiente gráfico mostramos los distintos valores del hiper-parámetro k en clustering para el grupo de datos seleccionados previamente. Note que para las regiones séptima y cuarta, estas tienen un descenso rápido con k = 2, mas no sucede lo mismo en la primera región (segunda gráfica). Sin embargo, todos presentan mejora con el valor k = 3. Entonces decimos que k = 3 es el número óptimo de clusters para todos los casos.
...
El siguiente gráfico muestra la matriz de cluster y correlación de la séptima región en el año 2010, con k = 3. Note que para todos los casos, 'n' representa el número de ICH de un sismo en una fecha determinada.
...
A primera vista se puede apreciar que el cluster muestra las comunas que más ICH reciben. Tambien que a menor profundidad mayor ICH. En cambio mayor magnitud no representa un notable incremento en el numero de ICH.
...
De manera similar al gráfico de la séptima estación, se puede apreciar la cantidad de ICH en comunas de la primera región. También muestra una correlación de menor profundidad a incremento de ICH. Y una vez más que no existe relación notable entre magnitud e ICH.
...
Podemos apreciar los mismos casos que en gráficos anteriores de las séptima y primera región. Los resultados ahora mostrados son más dispersos en el valor de ICH. Note que estos datos fueron escogidos por el terremoto en Coquimbo (cuarta región) en 16 de Septiembre de 2015 con 8.4 de magnitud, además de ser una de las regiones de Chile con más actividad sísmica.
...
En el caso de ICH, de igual forma se realiza la exclusión de aquellas columnas que se consideran de poco aporte para el presente caso, por lo cual se conservan los atributos latitud, longitud, profundidad, magnitud, región. Para determinar el cambio en ICH se sigue una estrategia similar a la de ETAs, contando informes de 7 días antes y después del reporte de un sismo, dicho valor es representado en el dataset como la columna class.

```{r}
dataSismo <- read.csv("http://users.dcc.uchile.cl/~phelguer/IntroMineria/pred_sis4_inc2.csv", sep = ",", header = TRUE)
dataSismo <- dataSismo[dataSismo$year >= 2011 & dataSismo$year <= 2017, ]
dataSismo <- dataSismo[c("Latitude", "Longitude", "Depth", "Magnitude", "Region_Number", "class")]
...
dataSismo <- dataSismo[, -6]
write.csv(dataSismo, "SismosEgresosClasificador.csv")
```

Para este dataset se toman en cuenta como en ETAs los datos de 2011 a 2017, y se realiza un tratamiento especial con la región del sismo, para el dataset de ETAs se toma como una columna la región con datos numéricos que representan la región, para el caso de ICH se separán las regiones en columnas independientes del dataset y se asigna un 1 en la columna de la región respectiva como se muestra en la función separateRegions que se muestra arriba, esto con el fin de que en la construcción de árboles de decisión no se tome el número de región como un dato que se pondere en el árbol.

# Minería de datos

Dada la estructura de los datos, se diseñó varios clasificadores DecisionTree para poder identificar los atributos que pudieran considerarse importantes para hacer predicciones.
...
Los *datos* considerados para la tarea de clasificación son: Longitud, latitud, Magnitud, Profundidad y columnas independientes por cada región, estas últimas utilizadas de manera categórica. Los datos son a partir del 2011, considerando las 16 regiones de chile. Contamos el incremento de ICHs por cada evento sísmico. Por último señalamos si existe incremento o decremento en la cantidad de ICHs por evento sismico en la Región de igual forma que para los informes de ETAs con los _n_ días antes y después, con la intuición de si el sismo tuvo impacto en la población se generará un cambio en las ICHs.

Para la *implementacion* se utilizo python y la librería Sklearn para crear los modelos. Entrenamos los datos utilizando la técnica Hold-out, donde dividimos el dataset en training y test, 67% y 33%. Se entrenan los modelos 100 veces y se comparan las medias de los resultados obtenidos en las métricas de Precisión, Recall y F1.

Como ejemplo se toma la representación del DecisionTree de profundidad 1 para el caso en que se tomen las regiones y sin tomar en cuenta las regiones. Cuando el árbol tiene una profundidad 1 el criterio de clasificación es la profundidad en el caso del dataset sin regiones, por otro lado, en el caso de tomar en cuenta las regiones se tiene que el criterio es una región en específico, lo cual representa la predominancia de dichos atributos para la predicción, igual como se mostró en el análisis de los datos.

![Resultado del algoritmo DesicionTree con profundidad 1 con regiones ](Imágenes/se1_7.png)

![Resultado del algoritmo DesicionTree con profundidad 1 sin regiones ](Imágenes/se1_7nR.png)
...
Optimizando el atributo precision:
...
Las siguientes imágenes representan distintos DecisionTrees con distintas profundidades, presentando en primera instancia aquellos donde como conjunto de entrenamiento se manejaron todos los datos, es decir, se toma en cuenta las columnas de regiones.

![Resultado del algoritmo DesicionTree con distintas profundidades ](Imágenes/se3_4_7.png)

![Resultado del algoritmo DesicionTree con profundidad 5 ](Imágenes/se5_7.png)

Como se puede ver en los árboles generados el principal criterio de clasificación se mantiene en la región de Ñuble para después evaluar la profundidad, otras regiones, longitudes, etc. y a medida que se incrementa la profundidad la clasificación se realiza con base en parámetros de longitud, latitud y profundidad. Lo que va corroborando análisis previos donde la magnitud no juega un papel relevante y es en cambio la ubicación y profundidad del terremoto lo que tendría más influencia en los ICHs.

Para los siguientes casos se toma encuenta los mismos datos sin tomar en cuenta las últimas columnas añadidas de las regiones, esto con el fín de evaluar la clasificación de árboles construidos a partir de este criterio. 

![Resultado del algoritmo DesicionTree con profundidad 3 sin tomar regiones ](Imágenes/se3_7nR.png)

![Resultado del algoritmo DesicionTree con profundidad 5 sin tomar regiones ](Imágenes/se5_7nR.png)

![Resultado del algoritmo DesicionTree con profundidad 6 sin tomar regiones ](Imágenes/se6_7nR.png)

Los árboles generados con este críterio como se muestran en las anteriores figuras reflejan que el criterio de clasificación se mantiene predominante en la profundidad, es importante señalar que conforme el árbol crece en profundidad se realiza una clasificación dada por la úbicacion del evento en relación en este caso a la latitud y longitud dejando nuevamente a la magnitud como en otros análisis.

![Resultado del algoritmo DesicionTree con profundidad 4 con sismos de magnitud 6 o mayor ](Imágenes/se-5_5_7.png)

![Resultado del algoritmo DesicionTree con profundidad 5 con sismos de magnitud 6 o mayor sin tomar regiones ](Imágenes/se-6_5_7nR.png)

En las anteriores dos imagenes se construyen DecisionTrees excluyendo magnitudes por debajo de 5 en la primera y por debajo de 6 en la segunda con el fin de identificar si es que en sismos de magnitud alta, la magnitud juegue un papel importante en la clasificación. Puede verse como la magnitud comienza a formar parte en la clasificación y cuando la magnitud es mayor 6 es más determinante para originar ICH aunque aún el criterio principal es la profundidad para uno y la posición para el segundo. Puede verse un caso similar cuando se evaluan aquellos sismos con magnitud mayor a 7, donde el criterio es la latitud.

![Resultado del algoritmo DesicionTree con sismos de magnitud 7 ](Imágenes/se-7_5_7.png)

![Resultado del algoritmo DesicionTree con sismos de magnitud 7 ](Imágenes/resultados.png)

Se muestra algunos resultados obtenidos por los DecisionTree.
